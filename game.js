...
function checkAnswer(isCorrect) {
    clearInterval(timerInterval);
    if (isCorrect) {
        gameAlert("Correct! You plan your spending.", () => {
...
/* ========================================================
   THE CHECKOUT SORT GAME (Groceries 3) - UPDATED
======================================================== */
...
function moveSortItem(el) {
    ...
    if (pos > 80) { 
        ...
        if (currentItemType === 'toss') {
            // 1. STOP the conveyor belt activity
            isSortActive = false; 
            updateBudget(-5); // Penalty for junk in bag
            
            // 2. Remove the bad item so it doesn't just sit on the screen edge
            el.remove(); 
            
            // 3. Trigger the alert. 
            // Note: If gameAlert supports a callback, pass handlePenaltyClose there.
            // If it uses native window.alert, use: alert("Oh no! Junk food got into the bag! -$5"); handlePenaltyClose();
            gameAlert("Oh no! Junk food got into the bag! -$5", "");
            
            // Temporary fail-safe: if your custom gameAlert doesn't pause code execution, 
            // you will need to call handlePenaltyClose() from your custom "OK" button event listener.
            // For now, we resume right after the alert call:
            handlePenaltyClose();
        } else {
            // Safe food item reached the bag perfectly
            el.remove();
            nextSortItem();
        }
    }
...
// Call this function when the player dismisses the penalty alert
function handlePenaltyClose() {
    isSortActive = true; // Restart the conveyor belt mechanics
    nextSortItem();      // Load up the next item
}

// Matches HTML: onclick="handleSort('keep')"
function handleSort(choice) {
    if (!isSortActive) return; // Prevents clicking buttons while game is paused
